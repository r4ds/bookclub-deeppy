...
## Sequence-to-sequence learning {.smaller}
...
- RNNs pass all information through a **sequential loop** — like reading a book once and recalling it from memory
...
- Answer: Attention is all you need! ([Vaswani et al., 2017](https://arxiv.org/abs/1706.03762))

## Dot-product attention {.smaller}
...
## Query, key, value: attention as lookup {.smaller}
...
## Multi-head attention {.smaller}

![Each head attends in its own subspace; the heads are then concatenated (Figure 15.7).](images/ch15-fig15-07-multi-head-attention.png){fig-align="center" height="200" fig-alt="Diagram showing multiple attention heads each producing different weighted combinations of source tokens, concatenated into the output."}
...
- Scale scores by `1/sqrt(head_dim)` for stable softmax gradients
...
- **Feedforward** sublayer: two `Dense` layers with ReLU — adds nonlinearity that attention alone lacks
...
:::: notes
The causal mask has the same motivation as not using a bidirectional RNN for language modeling — discuss?
::::
...
- ~14M parameters (under **half** the RNN model)
...
## Sample translations {.smaller}

Real test-set outputs (`[start]`/`[end]` stripped):

**RNN seq2seq**

- *I think they're happy.* → "yo creo que son felices" ✓
- *I will tell you tomorrow.* → "te lo voy mañana a decir" (right words, scrambled order)
- *The prisoner was set free yesterday.* → "el plan fue ayer a un atasco" ✗

**Transformer** (my training run)

- *How far are you going?* → "qué tan lejos vas" ✓
- *Lie still.* → "mantenete quieto" ✓ (idiomatic)
- *The museum is open to the public.* → "el museo está abierto el museo" ✗ (loops, drops "to the public")

:::: notes
Both models handle short, common sentences well and stumble on longer ones — and the failures are *confidently* wrong, not blank (here the Transformer loops "el museo" and drops half the sentence). That's why next-token accuracy (65–67%) is a poor proxy for quality, and real MT uses BLEU. Prompt: which failure is worse — the RNN's scrambled word order, or the Transformer looping and dropping content?
::::
...
- Language models learn **p(token | past tokens)**
...
- Hardware matters: parallel attention takes full advantage of modern GPUs
