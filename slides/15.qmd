...
## Book club timing: Colab T4 (cheapest tier)
...
:::: notes
Good discussion point: why is translation slower per step despite similar ms/step numbers? Batch size, sequence handling, and model size all matter. Total wall time also depends on epochs/steps per epoch.
::::
...
## Book club timing: Modal A10G (32 GB, 2 cores)
...
:::: notes
Worth noting: comparing across hardware (T4 vs A10G) is not apples-to-apples. The chapter also reports Transformers train ~3× faster per epoch than RNNs on the same hardware.
::::
